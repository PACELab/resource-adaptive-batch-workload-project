...
int main(int argc,char** argv)
{
	...
	vm->movingWindow.push_back(currentMemory); 
    ...
    if((GUARD_STEP_SIZE*vm->stdeviation)<minGaurdMem)
    {
        vm->fgReserved = vm->mean + minGaurdMem;
    } 
    else
    {
	vm->fgReserved = vm->mean + GUARD_STEP_SIZE*vm->stdeviation; 
    }
...
vm->mean = vm->sum/vm->window_size;
    ...
    while(1)
    {
        ...
        //cout<<  vm->window_size << endl;

	vm->mean = vm->sum/vm->window_size;
        vm->stdeviation = sqrt((vm->sum2 / vm->window_size) - (vm->mean * vm->mean));

        double newGaurdMem = minGaurdMem > GUARD_STEP_SIZE*vm->stdeviation ? minGaurdMem : GUARD_STEP_SIZE*vm->stdeviation;
	gaurdMem = 0.95 * gaurdMem + 0.05 * newGaurdMem;

        double temp = vm->mean + gaurdMem;
        double predictedPeakabove = temp>vm->original_limit?vm->original_limit:temp;
        temp = (vm->mean - gaurdMem);//        double predictedPeakbelow = temp<0?0:temp;
        double predictedPeakbelow = temp;
	...
	cout<<vm->currentMemory<<","<<predictedPeakabove<<","<<predictedPeakbelow<<","<<vm->fgReserved<<","<<con->bgReserved<<","<<con->bgunused<<","<<violations<<","<<phasechanges<<endl;
        ...
        if(currentMemory >= fg_reserved_pct*(vm->fgReserved)) 
        {
            ...
            con->bgunused=0;
	    vm->movingWindow.clear(); 
	    vm->window_size=0;
	    vm->sum = 0;
	    vm->sum2 = 0;

	    //cout<<"making it zero" << endl;
	...
	*/
	...
	//Moving window rule
	//cout << windowSize << "," << vm->window_size << endl; 

	if(vm->window_size>=windowSize)
	{
		vm->sum -= vm->movingWindow[0];
        	vm->sum2 -= (vm->movingWindow[0]*vm->movingWindow[0]);
        	vm->movingWindow.erase(vm->movingWindow.begin());
		vm->movingWindow.push_back(currentMemory);
		vm->sum += currentMemory;
        	vm->sum2 += currentMemory*currentMemory;	
	}
	else
	{
		...
		vm->sum += currentMemory;
		vm->sum2 += currentMemory*currentMemory;
		vm->window_size=vm->window_size+1;
