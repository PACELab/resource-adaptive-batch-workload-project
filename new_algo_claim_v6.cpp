...
string getDockerContainerName() {
	string output =  runCommand("docker ps --format '{{.Names}}'"); 
...
//Change executor number in the file in the container
void changeBGResources(int execs, string filename) {

        //Read the current number of processes from the file
	string container = getDockerContainerName();
        string file_output = runCommand(("docker exec " + container + " bash -c \"cat " + filename + "\"").c_str());
        stringstream ss1(file_output);
        string procs;
        getline(ss1,procs,'\n');
        int num_processes = stoi(procs);
        
        //write the new value into the file
        string comm = "docker exec " + container + " bash -c  \"echo " + to_string(execs) + " > " + filename + "\"";
        long int now =  unix_timestamp();
        cout<<"Updating resouces of BG container to " + to_string(execs)  +  "  at " +  to_string(now)<<endl;

        runCommand(("docker exec  " + container + " bash -c  \"echo " + to_string(execs) + " > " + filename + "\"").c_str());
        string output = runCommand(("docker exec " + container + " bash -c \"pgrep [s]tress\"").c_str());
        comm = "docker exec " + container + " bash -c \"pgrep [s]tress\"";
        stringstream ss(output);
        vector<int> pids;
        string pid;
        ...
        //Compare and kill or span new stressors accordingly
        if(execs > num_processes) {
		execs = execs - num_processes;
                runCommand(("docker exec -d " + container + " bash -c \"/usr/bin/stress --vm "  + to_string(execs) + "  --vm-bytes 1000M &\"").c_str());
        }
        else if (execs  < num_processes) {
              cout<<"did I ever enter this loop??"<<endl;
              int to_be_killed = num_processes  - execs;
              for(int i = 0; i < to_be_killed ; i++) {
                        cout<<"killing stress thread with pid " + pids[i]<<endl; 
			runCommand(("docker exec " + container + " bash -c\"kill -9 " + to_string(pids[i]) + "\"").c_str());
              }
        }
        now =  unix_timestamp();
        cout<<"Updated resouces of BG container to " + to_string(execs)  +  " at " +  to_string(now)<<endl;
...
int main(int argc,char** argv)
{
    ...
    if(argc<=1)
    {
        cout<<"./a.out <container_reserved_memory_in_gb> <init_window_size> <PHASE_CHANGE_SIZE> <GUARD_STEP_SIZE> <RECLAM_PCT> <CONTAINER RECLAIM SIZE> <FG_RESERVED_CLOSENESS_PCT> <FILENAME>"<<endl;
        exit(0);
    }
    ...
    //Update the file
    int num_execs = con->bgReserved / container_reclaim_size;
    changeBGResources(num_execs, filename);
    ...
    con->bgunused = fmod(con->bgReserved,container_reclaim_size);
    con->bgReserved = con->bgReserved - con->bgunused;
    num_execs = con->bgReserved / container_reclaim_size;
    changeBGResources(num_execs, filename);
