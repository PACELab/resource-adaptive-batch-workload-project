...
// Fetch values from trace. The format is 'second <ingress + outgress value>''
bool fetchBandwidthVal(ifstream& infile, long *val1, long *val2)
{
    std::string line;
...
int main(int argc, char** argv)
{
    ...
    vm->original_limit = 125; // Units = Mbytes/sec (944/8) = 125
    vm->contMaxBW = 100;
    vmLowerBW = 10;
    if(argc <= 1){
    ...
    std::ifstream infile("trace_result.txt");
    long val1, val2;
    for(int i = 0; i < vm->window_size; i++){
        // double curBW = getTotalcurrentBandwidth();
        if(!fetchBandwidthVal(infile, &val1, &val2)){
            cout << "Violations: " << numViolations << " Phase Changes: " << numChanges << endl;
            return 0;
        }
        // Just to make the trace value usable
        double curBW = val2/100000;
        vm->sum += curBW;
        vm->sum2 += (curBW * curBW);
        vm->curBW = curBW;
        // vm->to_string();
    ...
    while(1){
        ...
        if( vm->curContBW < couldBeContBW ) {
            ...
            // cout << "Reducing container BW limit to: " << couldBeContBW << " Reduce BW suggested: " << reduceBWto << " BW lower limit: " << vm->origContainerBW << endl;
